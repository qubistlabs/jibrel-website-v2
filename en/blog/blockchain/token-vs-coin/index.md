...
**[Ethereum](https://www.ethereum.org):** A platform used to launch many ICOs (Initial Coin Offerings)  during 2017. Its native token, Ether (ETH) is used to maintain all transactions and is paid out to miners who support the network as well as project teams during ICOs. For example, during Jibrel's ICO, investors contributed Ether (ETH) and received Jibrel Network Token (JNT) in return.  
...
*   Governance Tokens: Governance tokens allow holders to vote on changes in the network that they support. Typically, the number of tokens one holds is proportional to the number of votes they have.
...
During the ICO mania of 2017, many ERC20 token projects saw their prices skyrocket. For teams that wanted to raise money fast, creating tokens instead of coins proved to be a wise choice. Since then, projects like [Icon](https://icon.foundation) and [Vechain](https://www.vechain.org) have migrated to their own blockchains by conducting [coin swaps](https://en.bitcoinwiki.org/wiki/Coin_Swap). During a coin swap, holders and users exchange their tokens for [mainnet](https://coincentral.com/what-is-a-mainnet/) coins. Such swaps are typically supported by major exchanges like Binance, making the process automatic for their users. Those keeping their tokens safe in [their wallets](https://jwallet.network), must manually exchange them. 
