    ...
    if (isInheritanceEnabled) {
        return tars.packages.streamCombiner(
            ...
            tars.helpers.filterFilesByPath([
                new RegExp(`\/src\/${tars.config.fs.componentsFolderName}\/`),
                /_[\w]+.(jade|pug)/
            ])
        );
    }
...
module.exports = () => {
    return gulp.task('html:compile-templates', () => {
        ...
        let compileError;
        let filesToCompile = [
            `./src/pages/**/*.${tars.templater.ext}`,
            `!./src/pages/**/_*.${tars.templater.ext}`
        ];

        if (templaterIsPugOrJade && tars.options.watch.isActive) {
            filesToCompile.push(
                `!./src/${tars.config.fs.componentsFolderName}/**/_*.${tars.templater.ext}`,
                `./src/${tars.config.fs.componentsFolderName}/**/*.${tars.templater.ext}`
            );
        }
