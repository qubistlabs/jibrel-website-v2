        ...
        function applyDownloadedParts(params) {
            return new Promise((resolveDownloadedPartsApplying, rejectDownloadedPartsApplying) => {
                ...
                if (params.type === 'templater') {
                    Promise
                        .all([
                            new Promise((resolve, reject) => {
                                ncp(
                                    `./.tmpTemplater/tars-${tars.templater.name}-${params.version}/src/pages`,
                                    './src/pages',
                                    error => {
                                        if (error) {
                                            return reject(error);
                            ...
                            new Promise((resolve, reject) => {
                                ...
                                ncp(
                                    `./.tmpTemplater/tars-${tars.templater.name}-${params.version}/src/components`,
                                    `./src/${componentsFolderName}`,
                                    error => {
                                        if (error) {
                                            return reject(error);
                                        }
                    ...
                    const downloadedPreprocPartsPath = `./.tmpPreproc/tars-${tars.cssPreproc.name}-${params.version}/src`;
                    Promise
                        .all([
                            new Promise((resolve, reject) => {
                                ncp(
                                    `${downloadedPreprocPartsPath}/static`,
                                    `./src/${tars.config.fs.staticFolderName}`,
                                    error => {
                                        if (error) {
                                            return reject(error);
                                        }
                            ...
                            new Promise((resolve, reject) => {
                                ncp(
                                    ...
                                    `./src/${tars.config.fs.componentsFolderName}`,
                                    error => {
                                        if (error) {
                                            return reject(error);
                                        }
        ...
        function generateStartScreen() {
            return new Promise(resolve => {
                ...
                if (tars.cli) {
                    ...
                    console.log('\n');
                    tars.say('Hi!');
                    tars.say('Let\'s create awesome src!');
                }
