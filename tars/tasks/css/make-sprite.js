...
module.exports = () => {
    ...
    return gulp.task('css:make-sprite', done => {
        ...
        const cssTemplatePath = `./src/${staticFolderName}/${preprocName}/sprite-generator-templates`;
        ...
        function actionsOnTaskSkipping() {
            return actionsOnSpriteTaskSkipping({
                blankFilePath: `./src/${staticFolderName}/${preprocName}/sprites-${preprocName}/sprite_96.${preprocExtension}`,
        ...
        for (let i = 0; i < dpiLength; i++) {
            spriteData.push(
                gulp.src(`./src/${staticFolderName}/${imagesFolderName}/sprite/${usedDpiArray[i]}dpi/*.png`)
                .pipe(plumber({
                    errorHandler(error) {
        ...
        return spriteData[0].css
                ...
                .pipe(gulp.dest(`./src/${staticFolderName}/${preprocName}/sprites-${preprocName}/`))
                .pipe(
                    notifier.success(
